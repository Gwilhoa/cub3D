...
/*   Updated: 2022/11/17 19:44:40 by gchatain         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_move(t_cub *cub)
{
	...
	if (cub->key.key_d == 1)
	{
		if (cub->map.map[(int)(cub->map.pos_x + cub->s_ray.diry
				* (cub->s_ray.speed * 2))][(int)cub->map.pos_y] == '0')
			cub->map.pos_x += cub->s_ray.diry * cub->s_ray.speed;
		if (cub->map.map[(int)cub->s_ray.posx][(int)(cub->s_ray.posy
			- cub->s_ray.dirx * (cub->s_ray.speed * 2))] == '0')
			cub->map.pos_y -= cub->s_ray.dirx * cub->s_ray.speed;
	}
	if (cub->key.key_a == 1)
	{
		if (cub->map.map[(int)(cub->map.pos_x - cub->s_ray.diry
