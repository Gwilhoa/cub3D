...
void	calcul_step_sidedist(t_cub *s_cub, t_ray *s_ray) //raystep
{
	if (s_ray->raydirx < 0)
	{
		s_ray->stepx = -1;
		s_ray->sidedistx = (s_cub->s_ray->posx - s_ray->mapx) * s_ray->deltadistx;
	}
    else
    {
    	s_ray->stepx = 1;
    	s_ray->sidedistx = (s_ray->mapx + 1.0 - s_cub->s_ray->posx) * s_ray->deltadistx;
    }
    if (s_ray->raydiry < 0)
    {
    	s_ray->stepy = -1;
    	s_ray->sidedisty = (s_cub->s_ray->posy - s_ray->mapy) * s_ray->deltadisty;
    }
    else
    {
    	s_ray->stepy = 1;
    	s_ray->sidedisty = (s_ray->mapy + 1.0 - s_cub->s_ray->posy) * s_ray->deltadisty;
    }
}
...
void ft_dda(t_cub *s_cub, t_ray *s_ray)
{
	while (s_ray->hit == 0)
    {
        ...
        if (s_cub->map->map[s_ray->mapx][s_ray->mapy] == '1') 
			s_ray->hit = 1;
	  }
...
void raytodraw(t_cub *s_cub, t_ray *s_ray)
{
	...
	if (s_ray->side == 0)
		...
		s_ray->perpwalldist = s_ray->sidedisty - s_ray->deltadisty;
...
void draw_line(t_cub *s_cub, t_ray *s_ray)
{
	...
	{
		my_mlx_pixel_put(s_cub->s_img->img, s_ray->x, i, BLUE);
		i++;
	}
	...
	while (i <= W_H)
	{
		my_mlx_pixel_put(s_cub->s_img->img, s_ray->x, i, GREY);
		i++;
	}
}
// ------- fin du rail casting ---- //